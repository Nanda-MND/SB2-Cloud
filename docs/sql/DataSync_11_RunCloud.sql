/*
  Install sync on ALL eligible tables — CLOUD (no capture triggers).
  Run in SSMS on Cloud after DataSync_11_AllTables_Install.sql
*/

SET NOCOUNT ON;
DECLARE @InstallCapture bit = 0;
DECLARE @t sysname;
DECLARE @cnt int = 0;
...
PRINT N'SyncInstall_AllTables (Cloud) completed. Tables processed: ' + CAST(@cnt AS nvarchar(10));
GO
