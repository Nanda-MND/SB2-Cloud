/*
  ============================================================================
  LOCAL SB1 ONLY — ALL transaction *Detail hard-delete sync (L2C)
  ============================================================================
  ERP UI physically DELETEs detail lines. Sync must:
    1) Capture Op=D on Local (DELETE trigger → L2C)
    2) SyncAgent passes @Operation=D
    3) SyncApply_Generic hard-DELETEs *Detail on Cloud (hardDeleteDetail)

  Prerequisite on THIS DB:
    docs/sql/DataSync_10_SyncApply_Generic.sql   (marker: hardDeleteDetail)
    docs/sql/DataSync_11_AllTables_Install.sql   (SyncInstall_Table) — if missing
...
PRINT N'============================================================';
PRINT N'Txn Detail hard-delete sync — LOCAL';
...
BEGIN
    RAISERROR(N'STOP: CLOUD DB. Use Deploy_TxnDetail_HardDeleteSync_CLOUD.sql', 16, 1);
    RETURN;
END
...
BEGIN
    RAISERROR(N'STOP: Run DataSync_10_SyncApply_Generic.sql on LOCAL first (need hardDeleteDetail).', 16, 1);
    RETURN;
END
...
BEGIN
    PRINT N'SKIP remaining LOCAL steps (prerequisite / wrong DB).';
    SET NOEXEC ON;
END
GO
...
/* ---- SyncConfig: Local capture ---- */
PRINT N'--- SyncConfig CaptureLocal=1 / CaptureCloud=0 ---';
...
VALUES(@n, 1, 1, 0, N''ID'', 100, 45, N''TxnDetail L2C hard-delete'')';
        BEGIN TRY
...
/* ---- SyncInstall_Table (DELETE → L2C Op=D) ---- */
PRINT N'--- SyncInstall_Table (DELETE capture) ---';
IF OBJECT_ID(N'dbo.SyncInstall_Table', N'P') IS NULL
    PRINT N'WARN: SyncInstall_Table missing — run DataSync_11_AllTables_Install.sql then re-run.';
ELSE
BEGIN
    DECLARE @t sysname;
...
/* Re-assert Local capture */
PRINT N'--- Re-assert CaptureLocal=1 / CaptureCloud=0 ---';
...
/* ---- Requeue L2C Op=D (7d) ---- */
PRINT N'--- Requeue L2C *Detail Op=D (7d) ---';
...
BEGIN
    ...
    ELSE IF EXISTS (
        ...
        WHERE c.TableName = @t
          AND c.IsEnabled = 1 AND c.CaptureLocal = 1 AND ISNULL(c.CaptureCloud, 0) = 0
    )
        SET @cap = N'OK';
...
PRINT N'============================================================';
PRINT N'LOCAL DONE → run Deploy_TxnDetail_HardDeleteSync_CLOUD.sql';
PRINT N'Then rebuild SyncAgent. Test: delete detail → Cloud line gone.';
PRINT N'============================================================';
GO
