@echo off
REM Install tray status on the Dev PC only (same folder as SB.exe).
REM Do NOT copy this to client PCs.
REM set /p must stay outside parentheses. A prompt that contains () breaks cmd.exe.

setlocal
set SRC=%~dp0bin\Release\SB.SyncStatus.exe
set ERP=D:\Dev\SB2-Cloud-Runtime

if not exist "%SRC%" goto NeedBuild

echo.
echo === SB Sync Status - Dev PC only ===
echo Other client PCs do not need this app.
echo.

if exist "%ERP%\DBConnection.ini" goto CheckErp

echo DBConnection.ini was not found in:
echo   %ERP%
echo.
set /p ERP=ERP folder - must contain DBConnection.ini: 

:CheckErp
echo %ERP% | findstr /I "MinnNandar site4now" >nul
if errorlevel 1 goto CheckIni
echo Refusing live/client ERP folder:
echo   %ERP%
echo Use D:\Dev\SB2-Cloud-Runtime\ (no .git). Do not use the git checkout D:\Dev\SB2-Cloud\
pause
exit /b 1

:CheckIni
if exist "%ERP%\DBConnection.ini" goto CopyTray
echo Missing DBConnection.ini in:
echo   %ERP%
echo Encrypt Dev/Test ini with SB.SyncAgent\SB2_Dev_EncryptAndOnce.ps1
echo Run docs\sql\SB2_Run_LocalBootstrap.ps1 first.
echo Passwords stay in the environment, not in git:
echo   SB2_DEV_LOCAL_SQL_PASSWORD
echo   SB2_TEST_CLOUD_SQL_PASSWORD
pause
exit /b 1

:NeedBuild
echo Build first: Build-SyncStatus.cmd
pause
exit /b 1

:CopyTray
echo Copy to %ERP%
copy /Y "%SRC%" "%ERP%\SB.SyncStatus.exe" >nul
if exist "%~dp0bin\Release\SB.SyncStatus.exe.config" (
  copy /Y "%~dp0bin\Release\SB.SyncStatus.exe.config" "%ERP%\SB.SyncStatus.exe.config" >nul
)

echo Starting tray app. It also registers Windows startup for this user.
start "" "%ERP%\SB.SyncStatus.exe"

echo.
echo Done. Look for the green / yellow / red dot next to the clock.
echo Left-click: status window
echo Right-click: menu - Start with Windows, Exit
echo.
pause
exit /b 0
